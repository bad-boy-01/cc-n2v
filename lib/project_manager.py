...
class ProjectManager:
    ...
    def create_project(self, name: str) -> Path:
        ...
        if not self.project_exists(name):
            self.create_project_metadata(name, name)
    ...
    async def generate_overview(self, project_name: str) -> Dict:
        """
        Generate project overview using local QwenClient.

        Replaces the former GeminiClient-based implementation (v2 refactor).
        Runs locally — no API key, no paid service, Kaggle-compatible.

        Args:
            project_name: Project name

        Returns:
            Overview dict with synopsis, genre, theme, world_setting, generated_at
        """
        ...
        client = get_qwen_client(llm="qwen2.5-7b", load_in_4bit=True)

        schema = ProjectOverview.model_json_schema()
        schema_str = __import__("json").dumps(schema, ensure_ascii=False, indent=2)

        prompt = (
            "Analyze the following novel content and extract key information.\n\n"
            f"Novel content:\n{source_content[:8000]}\n\n"
            f"Output ONLY valid JSON matching this schema:\n{schema_str}"
        )

        # Local inference via QwenClient
        overview_dict = client.generate_json(prompt, temperature=0.1, thinking=True)
        ...
        try:
            ...
            # Accept partial data if validation fails
            pass
        ...
        print("✅ Project overview generated and saved (via QwenClient)")
